...
const LoginCMS = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post("http://localhost:1000/api/cms/login", { username, password });

      console.log("Response Login:", res.data);

      // coba lihat apakah ada user
      if (res.data.user) {
        console.log("User dari backend:", res.data.user);
      } else {
        console.log("Tidak ada user di response, fallback ke input username");
      }

      const token = res.data.token;
      const accessId = res.data.user?.accessId ?? 99;

      localStorage.setItem("token", token);
      localStorage.setItem("username", res.data.user.username)
      localStorage.setItem("accessId", res.data.user.accessId);
