...
function KelasBisnisFormSection({ id }) {
  ...
  useEffect(() => {
    api
      ...
      .catch((error) => {
        console.error("Error fetching tingkat kesulitan data:", error);
      });
  }, []);
  ...
  useEffect(() => {
    api
      ...
      .catch((error) => {
        console.error("Error fetching Kategori Kelas:", error);
      });
  }, []);
  ...
  function tambahMateriHandler() {
    setKelasBisnisDetail((prevKelasBisnisDetail) => {
      const newKelasBisnisDetail = {
        ...prevKelasBisnisDetail,
        materis: prevKelasBisnisDetail.materis.map((materi) => {
          return { ...materi };
        }),
      };
      newKelasBisnisDetail.materis.push({
        materi: "",
        link: "",
        deskripsi: "",
      });
      return newKelasBisnisDetail;
    });
  }
    ...
    <div className="w-full md:w-9/12 2xl:w-4/5 flex flex-col grow-0 px-10 md:px-28 py-8">
      ...
      >
        ...
        <div className="my-4">
          ...
          {kelasBisnisDetail.materis.map((materi, index) => (
            <div
              key={index}
        ...
        >
          <button
            ...
            type="submit"
            className="p-[10px] w-[123px] flex justify-center bg-white border border-black rounded-[10px] gap-2"
            style={{ color: isSubmitting ? "gray" : "black" }}
          >
            {isSubmitting ? <Spinner color="black" /> : null}
            <p className="text-[18px] text-center  font-medium leading-[28px] text-black shrink-0">
